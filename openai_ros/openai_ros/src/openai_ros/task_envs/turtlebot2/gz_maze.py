...
register(
    id='GZMaze-v0',
    entry_point='openai_ros:task_envs.turtlebot2.gz_maze.GZMazeEnv',
    timestep_limit=timestep_limit_per_episode,
)


class GZMazeEnv(turtlebot2_env.TurtleBot2Env):
    def __init__(self):
        """
        ...
        self.n_observations = numpy.asarray(laser_scan.ranges).size
        ...
        self.observation_space = spaces.Box(0, numpy.inf,shape=numpy.asarray(laser_scan.ranges).shape)
    ...
    def _set_action(self, action):
        ...
        self.angular_speed = (action - 10) * self.max_ang_speed * 0.1
        # We tell TurtleBot2 the linear and angular speed to set to execute
        self.move_base(self.linear_speed,
                       self.angular_speed,
                       epsilon=0.05,
                       update_rate=10,
    ...
    def _get_obs(self):
        ...
        for i, item in enumerate(laser_scan.ranges):
            if (min_range > laser_scan.ranges[i] > 0):
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            reward = round(15*(self.max_ang_speed - abs(self.angular_speed) +0.0335), 2)
